...
def ratio_percentiles_max_5_old(data, index):
    i = 0
    monthly_data = data.groupby(pd.Grouper(key='timestamp',freq='MS'))
...
def ratio_percentiles_max_5(raw_data, index):
    ...
    result_time_series = pd.Series(index = index)

    accum_data = pd.DataFrame()
    for period in index: # replace period_index by a range from creation to end.

        # Get contributions per contributor (unsorted)
        # use accumulated data until period i
        if period in period_index:
            accum_data = accum_data.append(data.loc[period])
        contributions = contributions_per_author(accum_data)

        result_time_series[period] = get_ratio_max_5_for_period(contributions)
...
######### Cached versions ################
# This functions need input data to have being previously cooked using prepare_stats()


def users_new_cached(data, index):
    users = data.drop_duplicates('contributor_id')
    series = users.groupby(level='period').size()
    if index is not None:
        series = series.reindex(index, fill_value=0)
    return series


# @index should be calculated using stats.calculate_index_all_months()
def ratio_percentiles_max_5_cached(data, index):
